...
# .. Leave-one-out figure
# Specification 6
# .. Optimize over 1980-2001, CO2 per capita, no covariates, OECD & high income
# .. Running Synth
# .. Export results
# .. Generate results
# .. Balance tests
...
# For the UK, the post-treatment gap is 45 times larger than
...
ratio.MSE["GBR"]
(length(ratio.MSE) - which(sort(ratio.MSE) == ratio.MSE["GBR"]) +1 )/ length(ratio.MSE)
# For the UK, the post-treatment gap is 86 times larger than
# the pre-treatment gap.
# If we were to pick a country at random from this sample,
# the chances of obtaining a ratio as high as this one would be
# 1/25 = 0.04
...
ratio.MSE["GBR"]
(length(ratio.MSE) - which(sort(ratio.MSE) == ratio.MSE["GBR"]) +1 )/ length(ratio.MSE)
# For the UK, the post-treatment gap is 112 times larger than
# the pre-treatment gap.
# If we were to pick a country at random from this sample,
# the chances of obtaining a ratio as high as this one would be
# 1/25 = 0.04
...
# Exclude countries with 5 times higher MSPE than UK
colnames(placebo.results[, pre.MSE > 5*UK.pre.MSE])
# Exclude ARE, ATG, BEL, BHR, BHS, BMU, BRB, BRN, CAN, CHL, CYM, DEU, FRO
# HKG, HUN, ISL, ISR, KNA, KOR, LUX, MLT, NCL, OMN, PAN, PLW, POL
# PYF, QAT, SAU, SGP, SYC, TTO, URY, USA
...
ratio.MSE["GBR"]
(length(ratio.MSE) - which(sort(ratio.MSE) == ratio.MSE["GBR"]) +1 )/ length(ratio.MSE)
# For the UK, the post-treatment gap is 32 times larger than
# the pre-treatment gap.
# If we were to pick a country at random from this sample,
# the chances of obtaining a ratio as high as this one would be
# 3/51 = 0.05882353
...
# Exclude countries with 5 times higher MSPE than UK
colnames(placebo.results[, pre.MSE > 5*UK.pre.MSE])
# Exclude ABW, ARE, ARG, AUT, BEL, BHR, BHS, BMU, BRB, BRN, CHE, CHL, CYM
# CYP, DEU, ESP, FRA, FRO, HKG, HUN, IRL, ISL, ISR, JPN, KNA, KOR
# LUX, MAC, MLT, NCL, OMN, PAN, POL, PRT, PYF, QAT, SAU, SGP, SYC
# TTO, TUR, URY, USA
...
ratio.MSE["GBR"]
(length(ratio.MSE) - which(sort(ratio.MSE) == ratio.MSE["GBR"]) +1 )/ length(ratio.MSE)
# For the UK, the post-treatment gap is 687 times larger than
# the pre-treatment gap.
# If we were to pick a country at random from this sample,
# the chances of obtaining a ratio as high as this one would be
# 6/52 = 0.1153846
...
g <- ggplot(MSE %>%
  ...
  labs(title = "Empirical distribution of ratio",
       ...
       y = "Density") +
  geom_segment(x = MSE %>% filter(country == "GBR") %>% select(ratio) %>% pull,
               xend = MSE %>% filter(country == "GBR") %>% select(ratio) %>% pull + 5000,
               y = 0.001,
               yend = 0.0025,
               col = "darkorchid",
               arrow = arrow(ends = "first", type = "closed",
                             length = unit(0.1, "inches")),
               arrow.fill = "darkorchid") +
  geom_text(x = MSE %>% filter(country == "GBR") %>% select(ratio) %>% pull + 20000,
            y = 0.0030,
            label = "United Kingdom",
            col = "darkorchid",
            hjust = 1)
...
colnames(placebo.results[, pre.MSE > 5*UK.pre.MSE])
# Exclude ABW, ARE, ARG, AUT, BEL, BHR, BHS, BMU, BRB, BRN, CHE, CHL, CYM
# CYP, DEU, ESP, FRA, FRO, GRC, HKG, HUN, IRL, ISL, ISR, JPN, KNA
# KOR, LUX, MAC, MLT, NCL, OMN, PAN, POL, PRT, PYF, QAT, SAU, SGP
# SYC, TTO, TUR, URY, USA
...
ratio.MSE["GBR"]
(length(ratio.MSE) - which(sort(ratio.MSE) == ratio.MSE["GBR"]) +1 )/ length(ratio.MSE)
# For the UK, the post-treatment gap is 682 times larger than
# the pre-treatment gap.
# If we were to pick a country at random from this sample,
# the chances of obtaining a ratio as high as this one would be
# 6/52 = 0.1153846
